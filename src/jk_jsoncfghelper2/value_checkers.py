...
class AbstractValueChecker(object):
	...
	def check(self, scmgr, value):
		ret = [ (".".join(a), b) for a, b in list(self._compile(scmgr, [])) ]
		yield from ret
		if not ret:
			ret = [ (".".join(a), b) for a, b in list(self._check(scmgr, [], value)) ]
			yield from ret
	#

	def checkB(self, scmgr, value, printFunc = None) -> bool:
		bSuccess = True

		if not self._bCompiled:
			for path, msg in self._compile(scmgr, []):
				bSuccess = False
				if printFunc:
					printFunc(".".join(path), msg)
				else:
					print(".".join(path), msg)

		if bSuccess:
			for path, msg in self._check(scmgr, [], value):
				bSuccess = False
				if printFunc:
					printFunc(".".join(path), msg)
				else:
...
class AbstractNumericValueChecker(AbstractValueChecker):
	...
	def __init__(self, pythonTypeOrTypes, humanReadableTypeNames, minValue = None, maxValue = None, allowedValues:list = None, required:bool = True):
		super().__init__()
...
class IntValueChecker(AbstractNumericValueChecker):
	...
	def __init__(self, minValue:int = None, maxValue:int = None, allowedValues:list = None, required:bool = True):
		super().__init__(int, "integer", minValue, maxValue, allowedValues, required)
	#
...
class FloatValueChecker(AbstractNumericValueChecker):
	...
	def __init__(self, minValue:float = None, maxValue:float = None, allowedValues:list = None, required:bool = True):
		super().__init__((float, int), "float", minValue, maxValue, allowedValues, required)
	#
...
class StringValueChecker(AbstractValueChecker):
	...
	def __init__(self, minLength:int = None, maxLength:int = None, allowedValues:list = None, required:bool = True):
		super().__init__()
...
class ListValueChecker(AbstractValueChecker):
	...
	def __init__(self, minLength:int = None, maxLength:int = None, allowedElementTypes:list = None, required:bool = True):
		super().__init__()
...
class DictionaryValueChecker(AbstractValueChecker):

	def __init__(self, children:dict = None, required:bool = True, structType:str = None):
		super().__init__()
	...
	def cloneObject(self):
		ret = DictionaryValueChecker(self.children, self.required, self.structType)
...
class StructureCheckerManager(object):

	def __init__(self):
		self.__types = {
			"int": IntValueChecker(),
			"float": FloatValueChecker(),
			"str": StringValueChecker(),
			"bool": BooleanValueChecker(),
		}
	#
